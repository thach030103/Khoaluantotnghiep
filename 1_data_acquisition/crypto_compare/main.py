...
if __name__=='__main__':
    ...
    btc_price = get_data(feature, params, prefix=prefix).drop(columns=drop_columns)
    ...
    for i in currencies:
        ...
        data = get_data(feature, params, prefix=prefix).drop(columns=drop_columns)
    ...
    for i in exchanges:
        ...
        data = get_data(feature, params, prefix=prefix)
    ...
    btc_blockchain = (get_data(feature, params, prefix='btc_', itype=1)
    ...
    balances = get_balance_data(feature, params, prefix=prefix)
    ...
    for df in dataframes:
      if df is not None and not df.empty:
        # Reset index nếu có NaN hoặc không hợp lệ
        if df.index.name != 'time' or df.index.hasnans:
            if 'time' in df.columns:
                df = df.dropna(subset=['time']).set_index('time')
            else:
                df = df.reset_index(drop=True)

        # Loại trùng, sort index
    ...
    if not cleaned_dataframes:
        raise ValueError("❌ Không có DataFrame hợp lệ để concat")

    btc_data = pd.concat(cleaned_dataframes, axis=1)
    btc_data = btc_data[~btc_data.index.duplicated(keep='last')].sort_index()
    ...
    for i in index_names:
        ...
        prefix = 'index_' + i + '_'
        indices = pd.concat(
            [get_data(feature, params, prefix=prefix), indices], axis=1)
    ...
    btc_volatility = get_data(feature, params, prefix=prefix)
